...
const AgentTrack = ({ track }) => {
  const successPct =
    track.totalCount === 0
      ? 0
      : Math.round(
          ((track.totalCount - track.failedCount) / track.totalCount) * 100,
        );
  return (
    <div className="agent-row">
      <div className="agent-avatar" style={{ background: "#e8f0fe" }}>
        {track.icon}
      </div>
      ...
      <div className="agent-track">
        <div
          ...
          style={{
            width: `${successPct}%`,
            background: "linear-gradient(90deg,#1d6ef5,#0aa8a0)",
          }}
        ></div>
      </div>
      <div className="agent-pct" style={{ color: "#1d6ef5" }}>
        {successPct}%
      </div>
    ...
    <div className="pattern-row">
      ...
      <div className="pattern-bar-track">
        <div
          ...
          style={{ width: `${pattern.percentOfFailures}%` }}
        ></div>
      </div>
...
export default function GovernancePanel() {
  ...
  const confidenceChartOptions = useMemo(
    () => ({
      ...
      xAxis: {
        categories: governanceData?.rcaConfidences.map(
          (x) => x.confidenceRange,
        ),
        gridLineWidth: 0,
        lineWidth: 0,
  ...
  return (
    <div
      className={`agent-panel card-governance${selectedAgent === "governance" ? " visible" : ""}`}
      style={{ display: selectedAgent === "governance" ? "block" : "none" }}
    >
      ...
      <div className="panel-grid">
        <div className="panel-col-governance">
          ...
          <div className="card-body" id="pnlGovernanceAgentMetrics">
            {governanceData?.agentPerformances.map((x) => (
              <AgentTrack key={x.name} track={x} />
            ))}
          </div>
        </div>
        <div className="panel-col-governance">
          ...
          <div className="card-body" id="pnlGovernanceFailurePatterns">
            {governanceData?.failurePatterns.map((x) => (
              <AgentFailurePatterns key={x.patternName} pattern={x} />
            ))}
          </div>
        </div>
        <div className="panel-col-governance">
            ...
            >
              <div
                style={{
                  ...
                  border: "1px solid rgba(13,140,92,0.2)",
                  borderRadius: "7px",
                }}
              >
                <div
              ...
              <div
                style={{
                  ...
                  border: "1px solid rgba(224,45,70,0.2)",
                  borderRadius: "7px",
                }}
              >
                <div
                ...
                <div
                  id="lblLowConfidence"
                  style={{ fontSize: 22, fontWeight: 800, color: "var(--red)" }}
                >
                  {lowConfidencePct}%
                </div>
